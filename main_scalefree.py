...
def run_contagion_scale_free(n: int, seed: int, z: int, capital_buffer: float, q: float) -> int:
    if(z==0):
        return 1
...
for capital_buffer in interval_cb:
    ...
    for z in interval_z:
        ...
        for iter in range(n_iters):
            cnt_failed = run_contagion_scale_free(n, random.randint(0, 100000000), z, capital_buffer, q)
...
with open('data/data_scalefree_ab_maxdegreeattack.npy', 'wb') as f:
