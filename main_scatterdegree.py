...
def runcont(n: int, seed: int, z: int, capital_buffer: float, q: float, targetdeg):
    if(z==0):
        return 1
    ...
    grade_nodes_list = [node[0] for node in G.out_degree if node[1] == targetdeg]
    if(len(grade_nodes_list) == 0):
        return 0
    first_fail = grade_nodes_list[np.random.randint(low=0, high=len(grade_nodes_list))]
...
z = 3
step = 1
interval_tz = np.arange(0, 10, step)
...
id = 0
for tz in interval_tz:
    print(" Executing test for tz = ", tz)

    start = timer()

    for iter in range(n_iters):
        cnt_failed = runcont(n, random.randint(0, 100000000), z, capital_buffer, q, tz)
...
plt.plot(interval_tz, count_systemic)
plt.show()

with open('data/data_cntsyst_vs_degreeattack.npy', 'wb') as f:
